package com.example.cong.barcodescanningapp;
...
public class MainActivity extends AppCompatActivity {
    ...
    int check=0; //biến kiểm tra nút bấm nào đã được bấm để ghi lại nội dung vào EditText
    @Override
    ...
    private View.OnClickListener onScanName=new View.OnClickListener(){
        public void onClick(View v){
            if(v.getId()==R.id.scan_button_name){
                IntentIntegrator scanIntegrator= new IntentIntegrator(MainActivity.this); // Khi nhấn nút scan thì màn hình sẽ chuyển từ màn hình chính sang màn hình quét.
                check=1; // gán giá trị check = 1 để biết đây là nút ScanName
                scanIntegrator.initiateScan(); // Sau khi nhận được ảnh hàm này sẽ xử lý hình ảnh
            }
        }
    };
